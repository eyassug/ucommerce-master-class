...
task SetPathVars -description "Sets the local paths." {
    ...
    $properties["website_root_folder"] = "C:\inetpub\SCTest\Website"
    ...
    #	/Website
    #	/Data
    $properties["website_data_folder"] =  "c:\inetpub\SCTest\Website\App_Data\data"
}
...
task CopyDemoStoreFiles -description "Copy all the Demo Store files to the local website" {
    ...
    & robocopy "$src\..\tools\Unicorn" "$website_root\App_Config\Include" Serialization.config $options
